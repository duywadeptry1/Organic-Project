...
function ProductDetailPage() {
  ...
  return (
    <div className="bg-[#FDFBF7] dark:bg-stone-950 min-h-screen py-8 text-stone-800 dark:text-stone-100 transition-colors duration-200">
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/60 text-red-700 dark:text-red-300 p-8 rounded-2xl text-center max-w-lg mx-auto">
            <h3 className="font-bold text-lg">Product Not Found</h3>
            <p className="text-sm mt-1 text-red-600 dark:text-red-400">{error?.data?.message || error.error || 'Unable to load product.'}</p>
          ...
          <div className="space-y-8">
              ...
              <div className="flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-bold text-green-700 dark:text-green-400 uppercase tracking-widest bg-green-50 dark:bg-green-950/50 px-2.5 py-0.5 rounded-md border border-green-200/30 dark:border-green-800/40">
                      {product.brand || 'Organi Certified'}
                    </span>
                    <span className="text-xs text-stone-400 dark:text-stone-500">• Non-GMO</span>
                  </div>

                  <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-stone-900 dark:text-stone-50 tracking-tight mb-3">
                    {product.name}
                  </h1>
                  ...
                  <div className="flex items-center gap-2 mb-4 text-sm">
                    ...
                    <span className="text-stone-400 dark:text-stone-400 text-xs">
                      ({product.numReviews || 18} customer reviews)
                    </span>
                  </div>

                  {/* Price */}
                  <div className="flex items-baseline gap-2 mb-6 pb-6 border-b border-stone-100 dark:border-stone-800">
                    <span className="text-3xl sm:text-4xl font-black text-green-700 dark:text-green-400">
                      ${(product.price || 0).toFixed(2)}
                    </span>
                    <span className="text-stone-400 dark:text-stone-400 text-sm font-medium">tax included</span>
                  </div>
                ...
                {/* Purchase Actions */}
                <div className="pt-4 border-t border-stone-100 dark:border-stone-800 space-y-5">
                  
                  {/* Stock Status & Quantity */}
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                      <span className="text-xs font-semibold text-stone-500 dark:text-stone-400 block mb-1">Availability:</span>
                      {!isOutOfStock ? (
                        <span className="inline-flex items-center gap-1.5 text-xs font-bold text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-950/50 border border-green-200 dark:border-green-800/60 px-2.5 py-1 rounded-lg">
                          <span className="w-2 h-2 rounded-full bg-green-500"></span>
                          In Stock ({stock} available)
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800/60 px-2.5 py-1 rounded-lg">
                    ...
                    {!isOutOfStock && (
                      <div>
                        <span className="text-xs font-semibold text-stone-500 dark:text-stone-400 block mb-1">Quantity:</span>
                        <div className="flex items-center border border-stone-200 dark:border-stone-700 rounded-xl bg-stone-50 dark:bg-stone-800 p-1">
                          ...
                          <span className="w-10 text-center font-bold text-stone-900 dark:text-stone-100 text-sm">{qty}</span>
                          <button
                            type="button"
                  ...
                  {/* Toast Feedback */}
                  {addedNotice && (
                    <div className="bg-green-50 dark:bg-green-950/60 border border-green-300 dark:border-green-800 text-green-800 dark:text-green-300 text-xs font-bold px-4 py-2.5 rounded-xl flex items-center justify-between animate-fade-in">
                      <span>✓ Added {qty} × {product.name} to your shopping bag!</span>
                      <Link to="/cart" className="underline hover:text-green-900 dark:hover:text-green-200">View Bag</Link>
                    </div>
                  )}
            ...
            <div className="bg-white dark:bg-stone-900 p-6 sm:p-8 rounded-3xl shadow-xs border border-stone-200/80 dark:border-stone-800 transition-colors duration-200">
              ...
              <div className="py-6 text-sm leading-relaxed text-stone-600 dark:text-stone-300">
                {activeTab === 'description' ? (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                      <div className="p-3.5 rounded-xl bg-stone-50 dark:bg-stone-800/60 border border-stone-100 dark:border-stone-700/60">
                        <span className="font-bold text-stone-800 dark:text-stone-200 block mb-1">🌱 100% Non-GMO</span>
                        <span className="text-xs text-stone-500 dark:text-stone-400">Free from synthetic fertilizers or artificial ripeners.</span>
                      </div>
                      <div className="p-3.5 rounded-xl bg-stone-50 dark:bg-stone-800/60 border border-stone-100 dark:border-stone-700/60">
                        <span className="font-bold text-stone-800 dark:text-stone-200 block mb-1">🚜 Direct From Partner</span>
                        <span className="text-xs text-stone-500 dark:text-stone-400">Fair-trade certified with verified regenerative soil practices.</span>
                      </div>
                      <div className="p-3.5 rounded-xl bg-stone-50 dark:bg-stone-800/60 border border-stone-100 dark:border-stone-700/60">
                        <span className="font-bold text-stone-800 dark:text-stone-200 block mb-1">❄️ Cold-Chain Protected</span>
                        <span className="text-xs text-stone-500 dark:text-stone-400">Maintained in climate-regulated storage for maximum freshness.</span>
                      </div>
                    </div>
                  </div>
                ) : (
